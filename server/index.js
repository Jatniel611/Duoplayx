...
// Helper para verificar si la peticiÃ³n es local (PC/Desktop del usuario) o nube (Render)
...
//  - Target https â†’ 302 directo (el navegador carga solo, en nube y en local).
//  - Target http + pÃ¡gina servida por http (localhost/Electron) â†’ 302 directo.
//  - Target http + pÃ¡gina https (Render) â†’ proxy OBLIGATORIO (mixed content).
app.get('/api/stream-proxy', async (req, res) => {
  ...
  // Solo se fuerza proxy cuando un destino http se carga desde una pÃ¡gina https.
...
app.get('/api/hls-proxy', async (req, res) => {
  ...
  // En Render (no local), los segmentos binarios (.ts/.m4s/.mp4) se redirigen
  // directo (0% ancho de banda del servidor). Las playlists m3u8 SIEMPRE se
  // reescriben por el proxy para que los segmentos hereden la URL correcta.
  ...
  try {
    ...
    if (targetUrl.includes('.m3u8') || contentType.includes('mpegurl') || contentType.includes('application/x-mpegurl')) {
      ...
      const rewritten = m3u8Body.split('\n').map(line => {
        ...
        // Local: canalizar segmentos por el proxy (misma-origen para el navegador).
        // Render: URL directa del CDN (0% banda).
        return isLocal
...
// Endpoint de resoluciÃ³n universal de medios
app.post('/api/resolve-media', async (req, res) => {
  ...
  try {
    ...
    // 5. MediaFire (ExtracciÃ³n del botÃ³n #downloadButton)
    if (cleanUrl.includes('mediafire.com')) {
      ...
      try {
        ...
        if (btnMatch && btnMatch[1]) {
          console.log(`[MediaFire Extractor] Enlace directo extraÃ­do: ${btnMatch[1].substring(0, 60)}...`);
    ...
    // Formato no soportado (solo YouTube, MP4 directo, .m3u8 y Drive directo)
    return res.status(422).json({ error: 'Formato no soportado. Usa un enlace de YouTube, un MP4 directo (.mp4), un .m3u8 o un Google Drive de descarga directa.' });
...
// SOCKET.IO â€” Sala de Watch Party
// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
io.on('connection', (socket) => {
  ...
  socket.on('join_room', (data, callback) => {
    try {
      ...
      if (!roomId) {
        if (typeof callback === 'function') return callback({ success: false, error: 'Introduce un cÃ³digo de sala.' });
        return;
      }
      ...
      if (!room) {
        if (typeof callback === 'function') {
          return callback({ success: false, error: `La sala '${cleanRoomId}' no existe o expirÃ³.` });
        }
        return;
      }
  ...
  socket.on('change_media_source', (data, callback) => {
    ...
    if (!room || room.hostId !== socket.id) {
      if (typeof callback === 'function') callback({ success: false, error: 'Solo el Host puede cambiar la pelÃ­cula.' });
      return;
    }
    ...
    if (!changeResult) {
      if (typeof callback === 'function') callback({ success: false, error: 'URL no vÃ¡lida o no soportada.' });
      return;
    }
...
server.listen(PORT, () => {
  console.log(`\n===================================================`);
  console.log(`ðŸš€ SERVIDOR DUOPLAYX INICIADO`);
  console.log(`ðŸŒ Acceso local: http://localhost:${PORT}`);
  console.log(`ðŸŽ¬ Formatos: YouTube, MP4 directo (.mp4), .m3u8 y Google Drive directo`);
  console.log(`===================================================\n`);
});
